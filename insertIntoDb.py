...
with open('customer.csv', 'r') as file:
	...
	for row in reader:
		cursor.execute("INSERT INTO mediAssist_customer  VALUES (?,?,?,?,?)" ,row)
		connection.commit()
		no_records += 1
...
# 	reader = csv.reader(file)
# 	for row in reader:
# 		cursor.execute("INSERT INTO mediAssist_vehicle  VALUES (?,?,?)" ,row)
# 		connection.commit()
# 		no_records += 1
#
...
# 	reader = csv.reader(file)
# 	for row in reader:
# 		cursor.execute("INSERT INTO mediAssist_policy  VALUES (?,?,?,?,?,?,?,?,?,?)" ,row)
# 		connection.commit()
# 		no_records += 1
#
